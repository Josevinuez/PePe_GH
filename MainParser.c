...
extern pp_void startParser(pp_void);
extern i32 startScanner(ReaderPointer sc_buf);

static pp_void printParserError(String fmt, ...);
static pp_void displayParser(ReaderPointer ptrBuffer);
static i64 getParserFileSize(String fname);
static pp_void callGarbageCollector(pp_void);
...
pp_void printParserError(String fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(pp_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
pp_void displayParser(ReaderPointer ptrBuffer) {
...
pp_void callGarbageCollector(pp_void) {
